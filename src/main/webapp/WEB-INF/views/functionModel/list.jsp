...
<body>
	...
	$(function(){
		...
		functionModelGrid.datagrid({
			...
			rownumbers:true,
			pagination:true,
			toolbar:"#tbFunctionModel",
			columns:[[
			          {field:'name',title:'权限名称',width:5},  
			          {field:'resourceAddr',title:'权限地址',width:5}
			]]
		});
...
// 				functionModelForm.form('reset');
			...
			findFunctionModel:function(){
				functionModelGrid.datagrid("load",{
					name: $("#Rename").val()
				}
			);
		},
		refreshFunctionModelGrid:function(){
			functionModelGrid.datagrid('reload');
		}
		};
		...
		$("a[data-functionModel]").on("click",function(){
			var functionModel = $(this).attr('data-functionModel');
			console.debug(functionModel);
			if(functionModel){
				functionModelObj[functionModel]();
			}
		});
	...
	<div id="tbFunctionModel">
		<a class="easyui-linkbutton" iconCls="icon-add" plain="true" data-functionModel='createFunctionModel' >添加</a>
		<a class="easyui-linkbutton" iconCls="icon-edit" plain="true" data-functionModel="editFunctionModel" >编辑</a>
		<a class="easyui-linkbutton" iconCls="icon-remove" plain="true" data-functionModel="deleteFunctionModel">删除</a>
		<a class="easyui-linkbutton" iconCls="icon-reload" plain="true" data-functionModel="refreshFunctionModelGrid">刷新</a><br/>
		<!-- 高级查询 -->
		<table>
			<tr>
				<td>权限名称:</td>
				<td><input id="Rename" name="name" type="text" style="width: 40px"/></td>
				<td><a class="easyui-linkbutton" iconCls="icon-search" plain="true" data-functionModel="findFunctionModel">查询</a></td>
			</tr>
		</table>
	</div>
	...
	<div id="functionModelDlg" class="easyui-dialog" style="width: 300px;height: 240px;">
		<form id="functionModelForm" method="post">
			<table align="center" style="margin-top: 10px">
				...
				<tr>
					<td>权限地址:</td>
					<td><input id="functionModelAddr" name="resourceAddr" type="text"/></td>
				</tr>
			</table>
		</form>
	</div>
